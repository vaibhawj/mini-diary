...
export default class Editor extends PureComponent<Props, State> {
	...
	static getStateFromEntry(
		...
		date: Moment,
		entryIdSelected: string | null
	): { textEditorState: EditorState; titleEditorState: EditorState, entryIdSelected: string | null } {

		const indexDate = toIndexDate(date);
		let text = "";
		let title = "";
		if (entryIdSelected) {
			const entry = entries[indexDate] && entries[indexDate].find(e => e.id === entryIdSelected);
	...
	render = (): ReactNode => {
		const { textEditorState, titleEditorState, dateSelected: dateInState, entryIdSelected: entryIdInState } = this.state;
