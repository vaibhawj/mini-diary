...
export function parseMiniDiaryJson(jsonStr: string): Entries {
	...
	const parsedEntries: ListDiaryEntry[] = Object.entries(parsedJson.entries).map(
		([indexDate, entriesForTheDay]) => entriesForTheDay.map(entry => { 
			return { indexDate, ...entry } 
		})).flat()
	...
	return buildEntries(parsedEntries, (parsedEntry: ImportEntry): {
		...
		const entry = { dateUpdated, title: title.trim(), text: text.trim(), id: id ? id : v4() };
		return { indexDate, entry };
	});
}
