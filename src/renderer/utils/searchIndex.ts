...
export async function createIndex(entries: Entries): Promise<void> {
	...
	const indexDocs = await Promise.all(
		Object.entries(entries).map(
			([indexDate, entriesForTheDay]) => entriesForTheDay.map(entry => createIndexDoc(indexDate, entry))
		).flat()
	);
...
export function searchIndex(key: string): SearchResult[] {
	const res = index
		...
		.map((searchResult: MiniSearchResult): SearchResult => ({ id: searchResult.id, indexDate: searchResult.indexDate}))
		.sort()
		.reverse();
	return res;
}
